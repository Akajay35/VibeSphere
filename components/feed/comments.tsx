...
type CommentRow = { id: string; body: string; created_at: string; profiles?: { username?: string; display_name?: string } | null };
...
export default function Comments({ postId }: { postId: string }) {
  ...
  async function submit() {
    ...
    if (!user) return;
    const { error } = await client.from('comments').insert({ post_id: postId, author_id: user.id, body: text });
    if (!error) { setBody(''); await load(client); }
  }
  ...
  return (
    <div className="mt-3 border-t pt-3">
      <div className="flex gap-2">
        ...
        <button onClick={() => void submit()} disabled={!supabase} className="rounded-full bg-[var(--brand)] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Post</button>
      </div>
