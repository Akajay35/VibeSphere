...
export default function MediaUploader() {
  ...
  async function publish() {
    if (!file) return;
    if (!supabase) {
      setMessage('Supabase is not configured. Add the production Supabase environment variables.');
      return;
    }
  ...
  return <section className="rounded-2xl border bg-white p-5 shadow-sm">
    ...
    <button disabled={!file || busy || !supabase} onClick={() => void publish()} className="mt-4 rounded-full bg-[var(--brand)] px-5 py-2.5 font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">{busy ? 'Uploading…' : !supabase ? 'Connecting…' : 'Publish'}</button>
