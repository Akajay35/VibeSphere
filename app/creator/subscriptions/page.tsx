...
export default function CreatorSubscriptionsPage() {
  ...
  async function load() {
    ...
    const { data } = await supabase.from('creator_plans').select('id,name,description,price_minor,currency,is_active').eq('creator_id', user.id).order('created_at', { ascending: false });
